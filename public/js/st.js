const st = {
    ...
    showConsole: function () {
        ...
        consoleEle.classList.add("show");
        this.darkModeStatus()
        this.asideStatus()
    },
    hideConsole: function (){
        const consoleEle = document.querySelector("#console")
        consoleEle && consoleEle.classList.remove("show");
    },
    //隐藏侧边栏
    ...
    musicToggle: function () {
        const $homeAplayerDom = document.querySelector('.home_player')
        if(!$homeAplayerDom) return
        const aplayer = $homeAplayerDom.aplayer
        
    },
    darkModeStatus: function(){
        let theme = document.documentElement.getAttribute('data-theme') === 'dark' ? 'dark' : 'light'
        const $consoleDarkDom = document.querySelector("#consoleDark")
        theme == 'dark' ? $consoleDarkDom.classList.add('on') : $consoleDarkDom.classList.remove('on')
    },
    asideStatus: function(){
        const $htmlDom = document.documentElement.classList
        const $asideDom = document.querySelector("#consoleHideAside")
        $htmlDom.contains('hide-aside') ? $asideDom.classList.remove("on") : $asideDom.classList.add("on")
    }
}
        ...
        fetch(fetchUrl)
            ...
            .then(json => {
                ...
                for (let i = 0; i < randomFriendLinks.length; ++i) {
                    ...
                    htmlText +=
                        `<a class='footer-item' href='${item.link}'  target="_blank" rel="noopener nofollow">${item.name}</a>`;
                }
